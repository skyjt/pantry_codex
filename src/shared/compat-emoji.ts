...
export function emojiSafePlainText(text: string, replacement = '[表情]'): string {
  const rendered = splitEmojiText(text)
    .map((part) => {
      if (part.emoji) return replacement
      return part.text.replace(GENERIC_EMOJI_RE, replacement).replace(EMOJI_JOINER_RE, '')
    })
    .join('')
