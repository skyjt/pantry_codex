...
**项目状态：v0.5.0 P1 本地交付候选**。v0.1–v0.4/P1 主链路已落地：发现、单聊/群聊、文件/图片/表情、截图标注、撤回、转发、群内 @、断点续传、迁移备份包、深色主题与核心设置均已实现；Windows 与 Debian 的真实打包运行测试按交付流程留给目标平台执行。进度以 `git log` 为准，里程碑规划见 [docs/tech-design.md](docs/tech-design.md) §12。
...
- **v0.4 / v0.5**：消息撤回、断点续传、聊天记录导出/导入（备份包含媒体）、深色主题、P1 设置补齐
...
技术选型与模块设计见 [docs/tech-design.md](docs/tech-design.md)。
...
- 内置头像与 emoji 兼容渲染使用 [Twemoji](https://github.com/jdecked/twemoji) SVG 图形子集，本地打包，图形许可为 CC-BY 4.0。完整声明见 [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md)。
