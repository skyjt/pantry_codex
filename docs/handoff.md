...
> 最后更新：2026-06-12（v0.5.7 输入框 hint 颜色修正候选）。**本文描述"当前状态与下一步"，会过期——以 `git log` 与各文档变更记录为准。**

## 0. 必读顺序（15 分钟上手）

1. **[AGENTS.md](../AGENTS.md)** —— 9 条硬性红线（Electron 22.3.27 焊死、纯内网、分层铁律等），违反即错误；
2. 本文 —— 状态、工作流、下一步；
3. 设计四件套（按需细读）：[requirements.md](requirements.md)（功能与 38 项决议）→ [protocol.md](protocol.md)（线上协议 v0.18）→ [ui-design.md](ui-design.md)（界面）→ [tech-design.md](tech-design.md)（选型/分层/库表）；
4. `git log --oneline` —— 提交历史就是完整开发史，每条 commit message 都是一份增量说明。

## 1. 项目状态一览

纯内网、无服务器、基于 IP 的局域网 IM + 文件传输（Electron 22 / Vue 3 / better-sqlite3）。
**v0.1、v0.2、v0.3、v0.4/P1 主链路已完成，本地进入 v0.5.7 交付候选**（对照 tech-design §12）。Windows / Debian 真实打包运行测试留给目标平台执行：
...
| UI | 三栏主窗、左侧 56px 茶青导航、三级通讯录树（公司▸部门▸团队）、资料卡+本地备注、私聊头部资料弹窗+备注编辑、全局搜索（FTS 按字）、发起讨论组两步搜索选人+设置、自绘 SVG 系统图标、明确齿轮设置入口、20 个亲和动物 emoji + 背景色头像模板、输入区图标中文延迟提示、输入框 placeholder 独立浅 hint 色、输入区最右侧会话内历史搜索（设置页尺度弹窗，默认显示最近记录，关键词/图片/文件/连续日期筛选，图片结果显示缩略图）、消息内容级右键菜单与边缘避让、历史滚动加载、设置独立小窗（桌面软件式分组面板）、首启向导、托盘+通知直达会话 |
...
- 决策落档：新决议追加到 requirements §9（编号已到 #38，续 #39+）+ 涉及文档的变更记录；协议改动必须 protocol.md 先行。
...
1. **本地五连验证**：本轮输入框 hint 颜色修正沿用 `npm test` → `npm run test:db` → `npm run typecheck` → `npm run build` → `PANTRY_UDP_PORT=47878 PANTRY_TCP_PORT=47879 npm run smoke`；后续改动仍需按同链路重跑，任何失败先修复再交付。
