...
| GitHub 发布 | 公开仓库：[skyjt/teahouse](https://github.com/skyjt/teahouse)；`.github/workflows/release.yml` 已配置 Windows 7 x64、Debian 10 / UOS 20 x64、Debian 10 / UOS 20 arm64、macOS arm64 四条发布线；Linux CI 在 Debian 10 容器内强制源码重建 better-sqlite3，electron-builder 关闭二次 `npmRebuild`，并检查源码重建产物与最终包内 `.node` 最高 GLIBC 符号不超过 2.28；arm64 使用 GitHub 远程 `ubuntu-22.04-arm` runner 跑 Debian 10 arm64 容器，产出 deb/AppImage 与独立 SHA 清单；push `main` / 手动触发上传 artifact，推送 `v*` tag 自动创建/更新 GitHub Release；目标平台真实桌面冒烟仍按 packaging-test 执行 |
